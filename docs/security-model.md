...
The model is untrusted input, not a security boundary. Every response is parsed as one discriminated Zod action. The active agent's in-code permission set is checked before registry lookup, unregistered tools are rejected, malformed responses receive bounded correction retries, repeated calls and excessive steps terminate, and final results are checked against role-specific schemas.
